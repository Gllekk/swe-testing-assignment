...
The two frameworks that were initially considered for running unit and integration tests are **JUnit 5** and **TestNG**. The two frameworks, despite both being capable at handling module and integration tests differ in their feature sets. 

**JUnit 5** is the more widespread option when it comes to Java testing. JUnit's main strengths are its architecture, which splits the framework into the API, engine, and launcher components, as well as a large number of annotations, such as `@Nested` or `@DisplayName`. Further, JUnit has great support for all the popular build tools, such as Maven or Gradle, as well as IDEs, like IntelliJ or VSCode, meaning that configuration can go very smoothly. Additionally, its `assertThrows` method makes the process of testing exceptions quite simple and clean.
...
For this project, **JUnit 5 was chosen over TestNG** due to the fact that the test suit is quite simple and straightforward, consisting of isolated unit tests and only a few integration tests. Using JUnit's `@Nested` classes allows to keep the file structure well-organized, without excessive XML configuration, while the `assertThrows` makes it possible to visually separate the division by zero from other cases and keep it readable. In the same conditions, TestNG's additional flexibility would remain unused and would add unnecessary complexity. As such, JUnit 5 was chosen for this project due to it being more suitable for the project's unique characteristics.
